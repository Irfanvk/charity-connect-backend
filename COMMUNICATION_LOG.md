...
|------|----------|-------|--------|-------|
| 2026-08-31 | Outstanding receivables now reflects missing active-member monthly dues | Both | ✅ | Production historical data records paid months as approved challans but often has no row for unpaid months. `GET /challans/outstanding` now applies the same payable-month policy used by member payments: it includes active members' missing months through the current month in addition to physical pending/generated challans. Derived rows are response-only (`is_inferred_due=true`); no records are created or existing payments changed. |
| 2026-08-31 | Celery delivery workflow hardened for non-blocking notifications and request moderation | Backend | ✅ | Persisted notifications now publish realtime events immediately and queue Web Push delivery to the `notifications` Celery queue when the broker is available; a direct-delivery fallback preserves behavior during broker outages. Celery workers consume `default,notifications` and use late acknowledgements, lost-worker rejection, error retention, and 45/60-second task limits. Member request creation now notifies active admins and superadmins. Production requires Redis, one worker, and exactly one Beat process; these services must share the same broker settings. |
| 2026-08-31 | Superadmin activity and notification-enrollment monitoring added | Both | ✅ | Backend records `last_seen_at` from authenticated activity (at most once per minute) and accepts an authenticated device-status report containing notification permission and browser/PWA display mode. New superadmin-only `GET /admin/user-monitoring` returns user activity, permission state, Web Push configuration state, and active device count without exposing push endpoints or keys. Frontend renders the data in the Superadmin Panel. `enabled` means at least one active push subscription; `disabled`, `not requested`, `needs resync`, `unsupported`, and `not reported` remain distinguishable. Data begins populating after each user opens the updated app. |
